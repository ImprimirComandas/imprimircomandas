...
const useComandaForm = (carregarComandas: () => Promise<void>, setSalvando: (value: boolean) => void) => {
  const [comanda, setComanda] = useState<Comanda>({
      ...
      valor_dinheiro: 0,
      valor_pix: 0,
      order_date: '', // Added the missing property
    });
  ...
  const onChange = (field: string, value: string | number | boolean | null) => {
    if (field === 'pesquisaProduto') setPesquisaProduto(value as string);
    else if (field === 'endereco') setComanda(prev => ({ ...prev, endereco: String(value) }));
    else if (field === 'pago') setComanda(prev => ({ ...prev, pago: Boolean(value) }));
  ...
  const salvarComanda = async () => {
    ...
    try {
      ...
      await import('../utils/printService').then(module => {
        module.imprimirComanda({ ...novaComanda, id: data.id, order_date: novaComanda.data });
      ...
      setComanda({
              ...
              valor_dinheiro: 0,
              valor_pix: 0,
              order_date: '', // Add the missing property
            });
      setQuantiapagaInput(null);
      ...
      toast.error(`Erro ao salvar comanda: ${errorMessage}`);
    } finally {
      setSalvando(false);
    }
  };
